    ...
    public class RedisCacheImpl : ICache
    {
        ...
        public RedisCacheImpl()
        {
            connectionMultiplexer = ConnectionMultiplexer.Connect(new ConfigurationOptions() {
                EndPoints = {
                    {ConfigurationManager.AppSettings["RedisHost"] , int.Parse(ConfigurationManager.AppSettings["RedisPort"]) }
                },
        ...
        public void Set(string key, string data, DateTime cacheTime)
        {
            var timeSpan = cacheTime - DateTime.Now;
